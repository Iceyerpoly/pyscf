    ...
    class QMMM(_QMMMGrad, grad_class):
        ...
        def grad_ewald(self, dm=None, with_mm=False, mm_ewald_pot=None, qm_ewald_pot=None):
            ...
            for i0, i1 in lib.prange(0, mol.natm, blksize):
                ...
                qm_ewovrl_grad[i0:i1] -= lib.einsum('i,ijx,j->ix',
                                                    qm_charges[i0:i1], Tija, all_mm_charges[mask])
                qm_ewovrl_grad[i0:i1] -= lib.einsum('ia,ijxa,j->ix',
                                                    qm_dipoles[i0:i1], Tijab, all_mm_charges[mask])
                qm_ewovrl_grad[i0:i1] -= lib.einsum('iab,ijxab,j->ix',
                                                    qm_quads[i0:i1], Tijabc, all_mm_charges[mask]) / 3
                if with_mm:
                    mm_ewovrl_grad[mask] += lib.einsum('i,ijx,j->jx',
                                                       qm_charges[i0:i1], Tija, all_mm_charges[mask])
                    mm_ewovrl_grad[mask] += lib.einsum('ia,ijxa,j->jx',
                                                       qm_dipoles[i0:i1], Tijab, all_mm_charges[mask])
                    mm_ewovrl_grad[mask] += lib.einsum('iab,ijxab,j->jx',
                                                       qm_quads[i0:i1], Tijabc, all_mm_charges[mask]) / 3
            ...
            qm_ewg_grad -= lib.einsum('i,gx,ig,g,g->ix',
                                      qm_charges, Gv, sinGvRqm, zcosGvRmm, Gpref, optimize=p)
            qm_ewg_grad += lib.einsum('i,gx,ig,g,g->ix',
                                      qm_charges, Gv, cosGvRqm, zsinGvRmm, Gpref, optimize=p)
            if with_mm:
                p = ['einsum_path', (0, 2), (1, 2), (0, 2), (0, 1)]
                mm_ewg_grad -= lib.einsum('i,gx,ig,g,g->ix',
                                          mm_charges, Gv, sinGvRmm, zcosGvRqm, Gpref, optimize=p)
                mm_ewg_grad += lib.einsum('i,gx,ig,g,g->ix',
                                          mm_charges, Gv, cosGvRmm, zsinGvRqm, Gpref, optimize=p)
            # qm dip - mm pc
            p = ['einsum_path', (4, 5), (1, 4), (0, 1), (0, 2), (0, 1)]
            qm_ewg_grad -= lib.einsum('ia,gx,ga,ig,g,g->ix',
                                      qm_dipoles, Gv, Gv, sinGvRqm, zsinGvRmm, Gpref, optimize=p)
            qm_ewg_grad -= lib.einsum('ia,gx,ga,ig,g,g->ix',
                                      qm_dipoles, Gv, Gv, cosGvRqm, zcosGvRmm, Gpref, optimize=p)
            if with_mm:
                p = ['einsum_path', (1, 3), (0, 2), (0, 2), (0, 1)]
                mm_ewg_grad += lib.einsum('g,j,gx,jg,g->jx',
                                          DGcosGvRqm, mm_charges, Gv, cosGvRmm, Gpref, optimize=p)
                mm_ewg_grad += lib.einsum('g,j,gx,jg,g->jx',
                                          DGsinGvRqm, mm_charges, Gv, sinGvRmm, Gpref, optimize=p)
            # qm quad - mm pc
            p = ['einsum_path', (5, 6), (0, 5), (0, 2), (2, 3), (1, 2), (0, 1)]
            qm_ewg_grad += lib.einsum('ga,gb,iab,gx,ig,g,g->ix',
            ...
            p = ['einsum_path', (3, 4), (1, 3), (1, 2), (0, 1)]
            qm_ewg_grad -= lib.einsum('i,gx,ig,g,g->ix',
                                      qm_charges, Gv, sinGvRqm, zcosGvRqm, Gpref, optimize=p)
            qm_ewg_grad += lib.einsum('i,gx,ig,g,g->ix',
                                      qm_charges, Gv, cosGvRqm, zsinGvRqm, Gpref, optimize=p)
            # qm pc - qm dip
            qm_ewg_grad += lib.einsum('i,gx,ig,g,g->ix',
                                      qm_charges, Gv, cosGvRqm, DGcosGvRqm, Gpref, optimize=p)
            qm_ewg_grad += lib.einsum('i,gx,ig,g,g->ix',
                                      qm_charges, Gv, sinGvRqm, DGsinGvRqm, Gpref, optimize=p)
            p = ['einsum_path', (3, 5), (1, 4), (1, 3), (1, 2), (0, 1)]
            qm_ewg_grad -= lib.einsum('ja,ga,gx,g,jg,g->jx',
                                      qm_dipoles, Gv, Gv, zsinGvRqm, sinGvRqm, Gpref, optimize=p)
            qm_ewg_grad -= lib.einsum('ja,ga,gx,g,jg,g->jx',
                                      qm_dipoles, Gv, Gv, zcosGvRqm, cosGvRqm, Gpref, optimize=p)
            # qm dip - qm dip
            p = ['einsum_path', (4, 5), (1, 4), (1, 3), (1, 2), (0, 1)]
            qm_ewg_grad -= lib.einsum('ia,ga,gx,ig,g,g->ix',
                                      qm_dipoles, Gv, Gv, sinGvRqm, DGcosGvRqm, Gpref, optimize=p)
            qm_ewg_grad += lib.einsum('ia,ga,gx,ig,g,g->ix',
                                      qm_dipoles, Gv, Gv, cosGvRqm, DGsinGvRqm, Gpref, optimize=p)
            # qm pc - qm quad
            p = ['einsum_path', (3, 4), (1, 3), (1, 2), (0, 1)]
            qm_ewg_grad += lib.einsum('i,gx,ig,g,g->ix',
                                      qm_charges, Gv, sinGvRqm, TGGcosGvRqm, Gpref, optimize=p) / 3
            qm_ewg_grad -= lib.einsum('i,gx,ig,g,g->ix',
                                      qm_charges, Gv, cosGvRqm, TGGsinGvRqm, Gpref, optimize=p) / 3
            p = ['einsum_path', (4, 6), (1, 5), (1, 2), (2, 3), (1, 2), (0, 1)]
            qm_ewg_grad += lib.einsum('jab,ga,gb,gx,g,jg,g->jx',
