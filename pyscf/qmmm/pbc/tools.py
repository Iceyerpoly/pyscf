...
# CPU-only mode: GPU helper removed
...
def get_qm_octupoles(mol, dm):
    ...
    for i in range(mol.natm):
        ...
        p0, p1 = aoslices[i, 2:]
        qm_octupoles.append(
            -contract('uv,xyzvu->xyz', dm[p0:p1], np.asarray(s1rrr)))
    qm_octupoles = np.asarray(qm_octupoles)
    return qm_octupoles
...
def estimate_error(mol, mm_coords, a, mm_charges, rcut_hcore, dm, precision=1e-8, unit='angstrom'):
    ...
    a = np.asarray(a)
    mm_coords = np.asarray(mm_coords)
    ...
    qm_coords = np.asarray(mol.atom_coords())
